...
class tabGUI:
    def __init__(self, rect, tabText):
        ...
        #position of this panel will not change, so rect has been predefined
    ...
    @staticmethod
    def dropDown(rect, options_list, starting_option, container):
    ...
    @staticmethod
    def label(rect, text, container): #doesn't have to be changed
...
class objects_tab(tabGUI):
    ...
    def handleMatrixEntry(self, event, entryIndex):
    ...
    def handleTextEntry(self, event, entryIndex):
        ...
        #sets text to the variable instead of newLine to reset the field if an erroneous value is entered
...
class camera_tab(tabGUI):
    ...
    def setText(self, keys):
        if keys[pygame.K_w] or keys[pygame.K_a] or keys[pygame.K_s] or keys[pygame.K_d] or keys[pygame.K_q] or keys[pygame.K_e] or cameraTurning:
            orientation = [round(n) for n in self.cam.orientation]
            self.inputVariables = [self.cam.position, orientation]
            self.variableEntries[0].set_text(str(self.inputVariables[0]))
            self.variableEntries[1].set_text(f'{self.inputVariables[1]}')
            self.updateMatrix()
...
scene = Scene(height)
scene.objects = [Sphere(Vec3.from_list([0, 0, 0]), [0, 0, 0], [0.5, 0.5, 0.5], name='Sphere1')]
...
def main():
    ...
    global cameraTurning, modelMoveFlag, objectChangeFlag
