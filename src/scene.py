...
class Scene:
    ...
    def full_transform(self):
        ...
        for model in self.objects:
            bunch = Matrix.bunch(model.vectors) #joins vectors together so only 1 matrix multiplication needed
    ...
    def get_tris(self): 
        ...
        tris = Model.clipTri.cull(tris)
        temp = np.array([depth(tri) for tri in tris])
        indexes = np.argsort(temp) #sorts ascending
        return indexes[::-1], tris
    ...
    def draw(self, indexes, tris, screen, bg):
        for i in indexes:
            points = self.align(tris[i].get2D())
            pygame.draw.polygon(screen, (255, 255, 255), points)
            pygame.draw.lines(screen, (0, 0, 0), True, points, 1)

    def getObjectNames(self):
        return [model.type for model in self.objects]
