...
class Model:
    ...
    def loadobj(self, path):
        """
        Loads vertex and face data for an object from a specified .obj file.

        Args: 
            path (str): path of .obj file

        Returns:
            vertices (list): list of all vertex coordinates
            tris (list): list of tuples specifying the vertices defining each tri. 
    ...
    def getValidTris(self): #finds the tris that are fully onscreen, returns list of Booleans
        validTris = []
        for tri in self.tris:
    ...
    class clipTri:
        ...
        def getSign(self): #returns the sign of the normal
        ...
        def cull(tris):
            ...
            return result                                       # the face points towards the camera (you can see it)
...
class Teapot(Model):
    def __init__(self, position:Vec3 = Vec3.from_list([0, 0, 0]), orientation:tuple = (0, 0, 0), scaling:tuple = (1, 1, 1), name='Object'):
        super().__init__(position, orientation, scaling, path='assets\teapot.txt', name=name)
        self.type = 'Teapot'
